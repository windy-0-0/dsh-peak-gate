...
const DEFAULT_CONFIG = {
  ...
  // 「忽略本次」的有效期：该对话在此毫秒数内不再被挂起（高峰不拦、网络失败也不接管）
  bypassMs: 600_000,
...
function matchesGate(options, cfg) {
  ...
  const provOk = cfg.match.providers.length === 0 || cfg.match.providers.some((p) => provider.includes(p))
  const modelOk = cfg.match.models.length === 0 || cfg.match.models.some((m) => model.includes(m))
...
function apply(ctx) {
  ...
  const bypasses = new Map() // sessionId → { until, label, at }：「忽略本次」免拦名单
  ...
  function isBypassed(sessionKey) {
    if (!sessionKey) return false
    const entry = bypasses.get(sessionKey)
    if (!entry) return false
    if (entry.until <= Date.now()) {
      bypasses.delete(sessionKey)
      return false
    }
    return true
  }
  ...
  function clearBypass(sessionKey) {
    ...
    if (sessionKey === undefined) {
      const n = bypasses.size
      bypasses.clear()
      return n
    }
    return bypasses.delete(sessionKey) ? 1 : 0
  }

  function bypassView() {
    pruneBypasses()
    const now = Date.now()
    return [...bypasses.entries()].map(([sessionId, entry]) => ({
  ...
  async function waitForValley(options, park) {
    ...
    while (true) {
      ...
      if (isBypassed(park.sessionKey)) return // 「忽略本次」：这个对话免拦，直接发
      ...
      if (!counted) {
        ...
        console.log(
          `[dsh-peak-gate] 高峰避让：挂起 ${park.provider}/${park.model} 请求（${park.label}），` +
            `预计 ${park.releaseText} 放行（当前 ${hhmm(now)}）；可在徽章里单独放行`,
        )
      }
      const releaseAt = nextReleaseAt(now, config)
  ...
  async function handleRequestError(payload, next) {
    try {
      ...
      if (isBypassed(sessionKeyOfPayload(payload))) return next()
  ...
  //   - action='bypass' → 「忽略本次」：放行它，并让**该对话**在 bypassMs 内完全免拦。
  function releaseParks({ id, kind, action } = {}) {
    ...
    for (const pool of pools) {
      for (const park of [...pool]) {
        ...
        if (action === 'bypass' && addBypass(park.sessionKey, park.label)) bypassed += 1
        if (releasePark(park)) {
...
const WIDGET_JS = [
  ...
  '    var bypInfo = state.bypass && state.bypass.length ? " · 已忽略 " + state.bypass.length + " 个对话" : ""',
  ...
  '    var act = make("button", null, p.kind === "network" ? "重试" : "放行")',
  '    applyStyle(act, {',
  ...
  // 「忽略本次」：这个对话放过去，并在有效期内不再被闸门拦（可撤销）
  '    var skip = make("button", null, "忽略本次")',
  '    applyStyle(skip, {',
  '      flex: "none", border: "1px solid #555", "border-radius": "6px", background: "transparent",',
  ...
  '    var info = make("div")',
  '    applyStyle(info, { flex: "1 1 auto", "min-width": "0" })',
  '    var head = make("div", null, b.label || "对话请求")',
  '    applyStyle(head, { overflow: "hidden", "text-overflow": "ellipsis", "white-space": "nowrap", color: "#bbb" })',
  '    var meta = make("div", null, "免拦剩余 " + fmtCountdown(b.remainMs) + (b.sessionId ? " · " + b.sessionId : ""))',
  ...
  '    }',
  '    panel.textContent = ""',
  '    var title = make("div", null, "挂起中的请求（" + list.length + "）· 放行 / 忽略本次")',
  '    applyStyle(title, { "font-weight": "bold", "margin-bottom": "2px" })',
  '    panel.appendChild(title)',
  '    var hint = make("div", null, "放行=只有这一次；忽略本次=这个对话在有效期内不再被拦")',
  ...
  '    if (byp.length) {',
  '      var sep = make("div", null, "已忽略 · 免拦中（" + byp.length + "）")',
