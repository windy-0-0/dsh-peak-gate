...
```
挂起中的请求（2）· 放行 / 忽略本次
放行=只有这一次；忽略本次=这个对话在有效期内不再被拦
[高峰] 帮我改一下插件，让断网也不中断        [放行] [忽略本次]
       deepseek-v4.1-flash · a1b2c3d4 · 12:00 自动放行
[网络] 帮我改一下插件，让断网也不中断        [重试] [忽略本次]
...
| 动作 | 含义 |
| --- | --- |
| **放行**（高峰） | 只放行**这一次**：不等低谷了，立刻按原价发出；后续请求照旧被闸门管住 |
| **重试**（网络） | 只立刻重连**这一次**；仍不通则继续挂起 |
| **忽略本次** | 放行这一次，并让**该对话**在 `bypassMs`（默认 10 分钟）内完全免拦——高峰不挂起、网络失败也不再接管。适合"我正在这个对话里干活，别冻我" |
| **恢复** | 撤销该对话的免拦，重新交给闸门管 |
...
- 免拦过期自动失效；`state.json` 的 `bypass[]` 可以看到还剩多久。
- 命令行等价写法：

```bash
# 只放行某一个（id 从 state.json 的 parks[].id 取）
...
| `bypassMs` | 「忽略本次」的免拦时长（毫秒，默认 600000 = 10 分钟；范围 1s–24h） |
...
DSH_HOME=$(mktemp -d) node test/release.test.mjs   # 行为：身份标签、按 id 放行、忽略本次免拦与撤销（16 项）
```
